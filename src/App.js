...
// Background Styling with Tailwind as an object
const style = {
...
const createTodo = async (e) => {
  e.preventDefault(e)
  if (input === '') {
    alert ('Please enter something meaninngful 😆🥱')
    return
  }
  ...
  return (
    <div className = {style.bg}>
        <div className = {style.container}>
          <h3 className = {style.heading}>Task Application</h3>
          ...
          {todos.length < 1 ? null : (
            <p className = {style.count}>{`You have ${todos.length} task(s) to complete`}</p>
          )}
        </div>
    </div>
  );
